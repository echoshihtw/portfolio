...
import { heroConfig } from "../src/content/portfolio.config";
...
test.describe("hero headline", () => {
  ...
  //
  // The expected text is derived from heroConfig rather than written out. The
  // first version hardcoded the headline of the day and failed the moment the
  // copy changed, which is a test asserting the wrong thing: the bug it was
  // written for is whitespace the markup adds, not the words themselves.
  test("headline has no stray whitespace", async ({ page }) => {
    await page.goto("/");
    const expected = heroConfig.headline
      .map((part) => part.text)
      .join("")
      .trim();
